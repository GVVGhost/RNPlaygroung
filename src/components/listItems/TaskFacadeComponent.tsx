      ...
      <CardWrapper style={{flexDirection: 'column'}}>
        <View style={{gap: Indent.L}}>
          ...
          <Text
            ...
            {taskContainer.description.length > 200
              ? taskContainer.description.substring(0, 200) + ' ...'
              : taskContainer.description}
          </Text>
          <View style={{gap: Indent.M, marginVertical: Indent.L}}>
            {(taskContainer.tasks.length < 3
              ? taskContainer.tasks
              : taskContainer.tasks.slice(0, 3)
            ).map((task, index) => (
              <View key={index}>
                <Text
                  style={{
            ...
            {taskContainer.tasks.length >= 3 && (
              <Text style={{paddingStart: Indent.XL, color: colors.text}}>
                ...
              </Text>
            )}
          </View>
          <View
            ...
            <Text
              ...
              Last edit{' '}
              {moment(taskContainer.updatedAt).format('DD/MM/YYYY hh:mm')}
            </Text>
